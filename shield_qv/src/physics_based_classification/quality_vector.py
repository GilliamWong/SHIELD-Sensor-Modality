...
def build_quality_vector(
    ...
    # Health score: mean |z-score| of mandatory + stable + sensitive features.
    # Keep the legacy raw distance available explicitly; the bounded
    # 0-1 health score is applied later via apply_health_score_calibration().
    health_feats = []
    seen_health = set()
    for feat in (
        quality_features.get('mandatory', [])
        + quality_features.get('stable', [])
        + quality_features.get('sensitive', [])
    ):
        if feat in qv_df.columns and feat not in seen_health:
            health_feats.append(feat)
            seen_health.add(feat)

    if health_feats:
        raw_health = qv_df[health_feats].abs().mean(axis=1)
        qv_df['qv_health_score_raw'] = raw_health
        qv_df['qv_health_score'] = raw_health
    ...
    if sensitive_feats:
        raw_degradation = qv_df[sensitive_feats].abs().mean(axis=1)
        qv_df['qv_degradation_score_raw'] = raw_degradation
        qv_df['qv_degradation_score'] = raw_degradation

    return qv_df


def fit_health_score_calibration(
    raw_scores: pd.Series,
    healthy_quantile: float = 0.95,
    min_scale: float = 1e-6,
) -> Dict[str, float]:
    """Fit a healthy-data envelope for the bounded 0-1 health score.

    The fitted envelope maps all healthy windows at or below the selected
    quantile to health=1.0. Scores above that envelope decay smoothly toward
    0 as their excess increases.
    """
    values = np.asarray(raw_scores, dtype=np.float64).reshape(-1)
    values = values[np.isfinite(values)]

    if values.size == 0:
        raise ValueError("Cannot fit health calibration with no finite scores.")

    quantile = float(np.clip(healthy_quantile, 0.5, 0.999))
    threshold = float(np.quantile(values, quantile))
    median = float(np.quantile(values, 0.5))
    iqr = float(np.quantile(values, 0.75) - np.quantile(values, 0.25))
    scale = max(threshold - median, iqr, min_scale)

    return {
        'healthy_quantile': quantile,
        'healthy_threshold': threshold,
        'healthy_scale': float(scale),
    }


def apply_health_score_calibration(
    qv_df: pd.DataFrame,
    calibration: Dict[str, float],
    source_col: str = 'qv_health_score_raw',
    target_col: str = 'qv_health_score',
) -> pd.DataFrame:
    """Apply a fitted healthy envelope and create a bounded 0-1 health score."""
    if qv_df.empty or source_col not in qv_df.columns:
        return qv_df.copy()

    threshold = float(calibration.get('healthy_threshold', 0.0))
    scale = float(calibration.get('healthy_scale', 1.0))
    scale = max(scale, 1e-12)

    result = qv_df.copy()
    excess = np.clip(result[source_col].to_numpy(dtype=np.float64) - threshold, 0.0, None)
    result['qv_health_excess'] = excess
    result[target_col] = 1.0 / (1.0 + excess / scale)
    result[target_col] = result[target_col].clip(0.0, 1.0)
