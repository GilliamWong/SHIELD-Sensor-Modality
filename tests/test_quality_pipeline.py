...
def test_wavelet_approximation_variance_is_centered():
    """Approximation variance should ignore DC level and measure spread only."""
    sig = np.ones(512) * 42.0
    feats = extract_wavelet_features(sig, fs=100.0, level=4)
    assert feats['modwt_a_energy'] > 0.0
    assert feats['modwt_a_variance'] < 1e-12
...
def test_cross_condition_stability_uses_sample_std():
    """Cross-condition CV should match the temporal stability ddof=1 convention."""
    df_a = pd.DataFrame({'window_start_sec': [0, 1], 'feat_shared': [1.0, 1.0]})
    df_b = pd.DataFrame({'window_start_sec': [0, 1], 'feat_shared': [3.0, 3.0]})
    stab = assess_cross_condition_stability({'a': df_a, 'b': df_b})
    cv = stab.loc[stab['feature'] == 'feat_shared', 'cv_across_conditions'].iloc[0]
    expected = np.sqrt(2.0) / 2.0
    assert abs(cv - expected) < 1e-12, f'CV={cv}, expected {expected}'
...
def test_health_score_monotonic_noise():
    ...
    Uses bias drift (not additive noise), because auto-selected stable
    features on white noise characterize noise *color* and don't respond
    to additive same-color noise. Bias drift changes the baseline and
    spectral shape, reliably shifting all feature categories.
    """
    np.random.seed(42)
    fs = 100.0
    ext = FeatureExtractor(fs=fs)
    clean = np.random.randn(10000)

    # Use 3.0s windows (300 samples) so MODWT level=5 is safe
    clean_feats = ext.process_signal(clean, 3.0, 1.5, include_signal_quality=True)
    stab = assess_feature_stability(clean_feats, n_segments=4)
    qf = select_quality_features(stab, n_stable=5, n_sensitive=3)
    ref_stats = {}
    for feat in qf['all']:
    ...
    moderate = clean + 3.0 * t ** 2
    mod_feats = ext.process_signal(moderate, 3.0, 1.5, include_signal_quality=True)
    qv_mod = build_quality_vector(mod_feats, qf, normalize=True, reference_stats=ref_stats)

    # Heavy drift
    heavy = clean + 8.0 * t ** 2
    hvy_feats = ext.process_signal(heavy, 3.0, 1.5, include_signal_quality=True)
    ...
    assert score_mod > score_clean, \
        f'Moderate drift health={score_mod:.3f} should > clean={score_clean:.3f}'
    assert score_hvy > score_mod, \
        f'Heavy drift health={score_hvy:.3f} should > moderate={score_mod:.3f}'
